...
- `Actions` (Frame, UIListLayout): holds the combat meters. Shown only while combat is active for the local player. Children `ShieldMeter` and `GunMeter`, each with `BarBG.Fill` (sized 0..1 by `CombatController`: shield charge / pistol cooldown) and `BarBG.EmptyOverlay` (visible while that meter is refilling from a cooldown).
