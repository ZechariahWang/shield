...
- Outside combat, players use the **stock Roblox classic camera and free cursor**. While combat mode is active (local state `Safe`, phase in `{Reveal, Round}`), `CameraController` takes the camera over (`CameraType = Scriptable`) and drives a shoulder view itself: yaw/pitch from mouse deltas (`CAMERA_SENSITIVITY` × the user's mouse sensitivity, pitch clamped to `[CAMERA_PITCH_MIN, CAMERA_PITCH_MAX]`), exponentially smoothed rotation and follow point (`CAMERA_ROTATION_SMOOTHING`, `CAMERA_FOLLOW_SMOOTHING`), `CAMERA_SHOULDER_OFFSET` applied in camera space around a pivot `CAMERA_FOCUS_HEIGHT` above the root part, mouse-wheel zoom clamped to `[CAMERA_MIN_ZOOM, CAMERA_MAX_ZOOM]`, an occlusion raycast that pulls the camera in front of walls (fast in, eased back out), and FOV eased to `CAMERA_COMBAT_FOV`. `RotationType = CameraRelative` so the Humanoid turns the character to face the camera. The step runs via `BindToRenderStep` at `RenderPriority.Camera + 1`. Everything restores to stock (`Custom` camera, `MovementRelative`, cursor visible) when it deactivates. The cursor is hidden (`MouseIconEnabled = false`) and the orange crosshair sits at the viewport center; aiming is a raycast from the camera through the viewport center.
