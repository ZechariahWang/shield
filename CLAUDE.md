...
- `Systems/AbilitySystem.luau`: dash mechanic for safe players (Q key). The dash goes along `Humanoid.MoveDirection` (WASD, camera-relative, replicated), or forward when standing still. Phase-gated via `start()` / `stop()`: dash is rejected outside the active `Round` phase, so safe players cannot dash during reveal or lobby. `start()` also clears `lastDash` so cooldowns don't carry across rounds.
...
1. After intermission, the 10s Reveal countdown begins: Status shows "GET READY!", secondary shows a positioning countdown. LMB does not fire a shot (no tracer, no `DamageDealt`/`DamageTaken`), E does not raise a shield, and Q does not dash: the `Actions` meters and the ability tile are visible but stay full and idle.
