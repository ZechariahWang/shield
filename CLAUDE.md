...
- `Main.client.luau`: bootstrap. Inits `AbilityHUDController`, `DashController`, `UIController`, `KillFeedController`, `KillConfirmController`, `CombatController`, `CombatFeedbackController`, `ChangeIndicatorController`, `ShopButtonController`, `CameraController`, `KillCamController` (in that order).
- Outside combat, players use the **stock Roblox classic camera and free cursor**. While combat mode is active (local state `Safe`, phase in `{Reveal, Round}`), `CameraController` takes the camera over (`CameraType = Scriptable`) and drives a shoulder view itself: yaw/pitch from mouse deltas (`CAMERA_SENSITIVITY_X` / `_Y` × the user's mouse sensitivity, pitch clamped to `[CAMERA_PITCH_MIN, CAMERA_PITCH_MAX]`), exponentially smoothed rotation and follow point (`CAMERA_ROTATION_SMOOTHING`, `CAMERA_FOLLOW_SMOOTHING`), `CAMERA_SHOULDER_OFFSET` applied in camera space around a pivot `CAMERA_FOCUS_HEIGHT` above the root part, mouse-wheel zoom clamped to `[CAMERA_MIN_ZOOM, CAMERA_MAX_ZOOM]`, an occlusion raycast that pulls the camera in front of walls (fast in, eased back out), and FOV eased to `CAMERA_COMBAT_FOV`. `RotationType = CameraRelative` so the Humanoid turns the character to face the camera. The step runs via `BindToRenderStep` at `RenderPriority.Camera + 1`. Everything restores to stock (`Custom` camera, `MovementRelative`, cursor visible) when it deactivates. The cursor is hidden (`MouseIconEnabled = false`) and the orange crosshair sits at the viewport center; aiming is a raycast from the camera through the viewport center.
- `Controllers/CameraController.luau`: the shoulder camera above. Exposes `setShake(offset)`: `CombatFeedbackController` feeds its damage jitter there and the camera adds it in camera space, so nothing else writes the camera CFrame during combat. `Humanoid.CameraOffset` is not used.
...
- `Crosshair` (Frame): small orange reticle pinned to the viewport center during combat (needs `AnchorPoint = 0.5, 0.5`; has a dark `UIStroke`); punched (size + color tween) by `CombatFeedbackController` on every shot fired.
