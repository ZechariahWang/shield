...
- `Main.client.luau`: bootstrap. Inits `AbilityHUDController`, `DashController`, `UIController`, `KillFeedController`, `KillConfirmController`, `CombatController`, `CombatFeedbackController`, `ChangeIndicatorController`, `ShopButtonController`, `KillCamController`, `SprintController`, `ShiftLockController` (in that order).
...
- `Controllers/SprintController.luau`: hold Shift to sprint while combat is active and the shield is down: `WalkSpeed` to `SPRINT_WALKSPEED` and the camera FOV eased from `CAMERA_FOV` to `SPRINT_FOV` over `SPRINT_FOV_TWEEN_TIME`, both restored on release. Client-side: the local character is client-simulated, and WalkSpeed is re-applied every frame because the server rewrites it on shield changes.
